...
describe("config io paths", () => {
  ...
  it("defaults to ~/.simpal/simpal.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
      });
      expect(io.configPath).toBe(path.join(home, ".simpal", "simpal.json"));
    });
  });
  ...
  it("uses OPENCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      ...
      expect(io.configPath).toBe(path.join(home, "svc-home", ".simpal", "simpal.json"));
    });
  });
